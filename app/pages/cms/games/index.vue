  ...
  <section class="container mx-auto px-4 py-6">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <button @click="openCreate" class="px-3 py-2 rounded bg-emerald-600 text-white text-sm">Tambah Game</button>
      </div>
    </div>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-for="g in items" :key="g.id" class="border-t">
            ...
            <td class="px-3 py-2">
              <div class="flex items-center gap-2">
                <button @click="goProducts(g)" class="px-2 py-1 border rounded" title="Products">📦</button>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/30 flex items-center justify-center p-4">
      <div class="bg-white rounded shadow w-full max-w-lg">
        <div class="px-4 py-3 border-b font-semibold">{{ editing ? 'Edit Game' : 'Tambah Game' }}</div>
        <div class="p-4 space-y-3">
          ...
          <div>
            ...
            <input v-model="form.slug" class="w-full border rounded px-2 py-1" />
          </div>
          <div>
...
async function fetchList() {
  loading.value = true
  try {
    const res: any = await $fetch('/api/cms/games', { params: { page: page.value, pageSize: pageSize.value } })
...
function goProducts(g: any) { router.push(`/cms/products/${g.id}`) }

// CRUD modal
const showForm = ref(false)
const editing = ref(false)
const currentId = ref<string | null>(null)
const form = reactive({ slug: '', name: '', publisher: '', image: '', is_active: true })

function openCreate() {
  editing.value = false
  currentId.value = null
  Object.assign(form, { slug: '', name: '', publisher: '', image: '', is_active: true })
  showForm.value = true
}
...
function openEdit(g: any) {
  ...
  Object.assign(form, { slug: g.slug, name: g.name, publisher: g.publisher, image: g.image, is_active: g.is_active })
  showForm.value = true
}
